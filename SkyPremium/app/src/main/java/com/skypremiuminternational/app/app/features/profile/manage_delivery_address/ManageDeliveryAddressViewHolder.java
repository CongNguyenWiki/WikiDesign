    ...
    if (address.getStreet() != null) {
      if (address.getStreet().size() > 0) {
        for (String street : address.getStreet()) {
          ...
          Log.d("street:", ""+streets);
        }
      }
    }

    if (streets.length() > 0) {
      String str = streets.toString();
      String fullAdd;
      fullAdd = str;
      ...
      if (address.getCity() != null && !TextUtils.isEmpty(address.getCity()) && !address.getCity().equalsIgnoreCase("-")) {
        fullAdd += "\n" + address.getCity();
      }
      if (stateName != null && !TextUtils.isEmpty(stateName) && !stateName.equalsIgnoreCase("-")) {
        fullAdd += " " + stateName;
      }
      if(address.getCountryId()!=null && !TextUtils.isEmpty(address.getCountryId()) && !address.getCountryId().equalsIgnoreCase("-")){
