...
import static com.skypremiuminternational.app.app.features.profile.my_reservation.confirm_create_reservation.ConfirmReservationDialogFragment.CONFIRM_DIALOG;
...
import com.skypremiuminternational.app.domain.models.reservation.ReserveTimeSlotItem;
...
public class MakeAReservationDialogFragment extends BaseBottomDialogFragment<MakeAReservationPresenter>
    implements MakeAReservationView<MakeAReservationPresenter>, DateSinglePickerDialogFragment.DateSinglePickerListener , View.OnFocusChangeListener {
...
/*  @SuppressLint("RestrictedApi")
  ...
  public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
    ...
    tvsChildrenNumber.getEdt().setOnFocusChangeListener(this);
    tvsAdultsNumber.getEdt().setOnFocusChangeListener(this);
  }
  ...
  void onClickSave(){
    ...
    reservationRequest.setNote(tvsSpecialInstruction.getText().trim() + " This is a Sky Premium Member Booking.");
  ...
  boolean isValid(){
    ...
    if(TextUtils.isEmpty(outletSelectedId)){
      tvsReservationTime.setError(tvsReservationTime.getHint());
      tvsReservationTime.showError();
      isValid = false;
    }

    if(TextUtils.isEmpty(selectedDate)){
      tvsReservationDate.setError(tvsReservationDate.getHint());
    ...
    if(TextUtils.isEmpty(tvsAdultsNumber.getText().trim())){
      tvsAdultsNumber.setError(tvsAdultsNumber.getHint());
    ...
    /**
     * if (totalGuest > maxCapacity){
     * 						"You have exceed the maximum seating capacity allowed. Please reduce the number of guests."
     *                    } else if (totalGuest < minCapacity){
     * 						"You have not met the minimum seating capacity. Please increase the number of guests."
     *          } else if (reservationAdult < 1){
     * 						"You need to have at least 1 adult."
     *          } else if(totalGuest > 5 && covid19) {
     * 						"Reservations are open for dining of up to 5 people per table, in accordance with the government health & safety guidelines. Reservations above 5 pax will be seated in separate table."
     *          }*/
    ...
    if(totalGuest > max){
      validMessage = "You have exceed the maximum seating capacity allowed. Please reduce the number of guests.";
      isValid = false;
    }else if(totalGuest < min){
      validMessage = "You have not met the minimum seating capacity. Please increase the number of guests.";
      isValid = false;
    }else if(numAdult < 1){
      validMessage = "You need to have at least 1 adult.";
      isValid = false;
    }/*else if(totalGuest > 5 && isCovid19){
      validMessage = "Reservations are open for dining of up to 5 people per table, in accordance with the government health & safety guidelines. Reservations above 5 pax will be seated in separate table.";
      isValid = false;
    }*/
...
/*    if(numAdult <= 0){
    ...
    if(totalGuest<=0  || totalGuest < min || totalGuest > max  ){
  ...
  private int getMaxCap() {
    ...
    list.add(reserveTimeSelected.getMaxNumberOfSeatsForSingleReservation() == null ? 0 : reserveTimeSelected.getMaxNumberOfSeatsForSingleReservation());
    list.add(reserveTimeSelected.getRemainingShiftCapacity()== null ? 0 : reserveTimeSelected.getRemainingShiftCapacity());
    list.add(reserveTimeSelected.getRemainingTimeSlotCapacity()== null ? 0 : reserveTimeSelected.getRemainingTimeSlotCapacity());
    list.add(reserveTimeSelected.getMaxPartySizeBooking()== null ? 0 : reserveTimeSelected.getMaxPartySizeBooking());
    ...
    list.add(reserveTimeSelected.getMinNumberOfSeatsForSingleReservation() == null ? 0 : reserveTimeSelected.getMinNumberOfSeatsForSingleReservation());
    list.add(reserveTimeSelected.getMinPartySizeBooking()== null ? 0 : reserveTimeSelected.getMinPartySizeBooking());
  ...
  private void logInformationReservation() {
    Log.d("RESERVATION_LOG_FORM" ,
            ...
            +"\ntvsSpecialInstruction:  " +tvsSpecialInstruction.getText() +" This is a Sky Premium Member Booking."
            +"\nProduct:  "               +productId
  ...
  void intitTimeSlotList(List<ReserveTimeSlotItem> reservationTimeList){
    ...
    for(int i = 0 ; i < reservationTimeList.size() ; i++){
      ...
      itemSlotsId[i] = reservationTimeList.get(i).getTimeSlotId();
    }
  ...
  public void onFocusChange(View v, boolean hasFocus) {
    ...
    int adults = (TextUtils.isEmpty(tvsAdultsNumber.getText().toString().trim()) ?
        0 : Integer.parseInt(tvsAdultsNumber.getText().toString().trim())) ;
    int childrent = (TextUtils.isEmpty(tvsChildrenNumber.getText().toString().trim()) ?
        0 : Integer.parseInt(tvsChildrenNumber.getText().toString().trim())) ;

    int sumPeople = adults  + childrent;


    if(tempAdult !=adults||tempChild!=childrent){
      if(sumPeople > 5 && isCovid19){
        ...
        /*new AlertDialog.Builder(getContext())
            .setMessage("Reservations are open for dining of up to 5 people per table, " +
                "in accordance with the government health & safety guidelines. " +
                "Reservations above 5 pax will be seated in separate table.").show();*/
      }else {
