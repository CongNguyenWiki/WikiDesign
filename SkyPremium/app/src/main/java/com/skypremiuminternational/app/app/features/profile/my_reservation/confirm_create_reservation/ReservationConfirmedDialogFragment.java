...
public class ReservationConfirmedDialogFragment extends DialogFragment {
  ...
  private String createCancelPolicy() {
    ...
    string =
        " <font color='#CCA255'><b>" +
            (TextUtils.isEmpty(reservationResult.getReserveResultDataItem().get(0).getSkyDollarEarn()) ? "0" : reservationResult.getReserveResultDataItem().get(0).getSkyDollarEarn()) +
        " " + res(R.string.sky_dollars) +
        " </b></font>" + res(R.string.msg_redden_sky_2) +
        " " + (TextUtils.isEmpty(reservationResult.getReserveResultDataItem().get(0).getLoyaltyExpired()) ? "0" : reservationResult.getReserveResultDataItem().get(0).getLoyaltyExpired()) +
  ...
  String createContentQuestion() {
    ...
    String timeSlot = reservationRequest.getTime();
