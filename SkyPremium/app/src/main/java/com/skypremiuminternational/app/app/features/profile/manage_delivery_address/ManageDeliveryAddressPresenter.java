...
import com.skypremiuminternational.app.data.network.request.AddressDeliveryRequest;
...
import com.skypremiuminternational.app.domain.interactor.countrycode.GetCountryCodes;
  ...
  void updateAddress(Address address, boolean addNewAddress,List<ISOCountry> isoCountryList,List<CountryCode> countryCodes) {
    ...
    */
    ...
    add(updateUserAddress.execute(new SingleSubscriber<UserDetailResponse>() {
      ...
      @Override
      public void onError(Throwable error) {
        if(request.getMember_id().isEmpty()){
          getView().hideLoading();
  ...
  private UpdateUserDeliveryRequest buildRequestParams(Address address,List<ISOCountry> isoCountryList,List<CountryCode> countryCodes) {
    ...
    for (CustomAttribute customAttribute : userDetailResponse.getCustomAttributes()) {
      if (customAttribute.getAttributeCode().equals("language")) {
        ...
        customAttributeList.add(new CustomAttribute("uid", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("sales_sv_code")) {
        customAttributeList.add(new CustomAttribute("sales_sv_code", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("sales_agency_code")) {
        customAttributeList.add(new CustomAttribute("sales_agency_code", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("sales_agent_code")) {
        customAttributeList.add(new CustomAttribute("sales_agent_code", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_country_number")) {
        customAttributeList.add(new CustomAttribute("registration_country_number", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_phone_number")) {
        customAttributeList.add(new CustomAttribute("registration_phone_number", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_country")) {
        customAttributeList.add(new CustomAttribute("registration_country", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("membership_temporary_offer_date")) {
        customAttributeList.add(new CustomAttribute("membership_temporary_offer_date", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_state")) {
        customAttributeList.add(new CustomAttribute("registration_state", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_postal_code")) {
        customAttributeList.add(new CustomAttribute("registration_postal_code", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_city")) {
        customAttributeList.add(new CustomAttribute("registration_city", customAttribute.getValue()));
      }
      else if (customAttribute.getAttributeCode().equalsIgnoreCase("registration_address1")) {
        customAttributeList.add(new CustomAttribute("registration_address1", customAttribute.getValue()));
      }
    ...
    for (int i = 0; i < addressList.size(); i++){
      ...
      addressList.get(i)
          .setPrefix(SalutationUtils.getSalutationCode(addressList.get(i).getPrefix()));

      addressDeliveryRequests.add(addressDeliveryRequest(addressList.get(i)));
    }
    customer.setAddresses(addressDeliveryRequests);
    return new UpdateUserDeliveryRequest(customer, userDetailResponse.getId().toString());
  }
