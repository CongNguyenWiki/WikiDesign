...
import com.skypremiuminternational.app.domain.utils.CountryUtil;
...
//  SkyTextInputLayout inputState;
  ...
  public void setupDialog(Dialog dialog, int style) {
    ...
    if (dialogType == ADD_ADDRESS) {
      ...
      if(countryCodes!=null) {
        for(CountryCode code : countryCodes){
          if(code.getId().equals("301")){
            inputCountry.setText(code.getName());
            selectedCountryId = Integer.parseInt(code.getId());
    ...
    inputCountry.edt.setClickable(false);
...
//    }
  ...
  private int getSalutationPosition(String prefix) {
    for (int i = 0; i < Constants.SALUTATIONS_NEW.length; i++) {
      if (prefix.equalsIgnoreCase(Constants.SALUTATIONS_NEW[i])) {
        return i;
      }
    }
    return -1;
...
//    }
  ...
  private Address buildAddress() {
    if (address != null) {
//      address.setPrefix(Constants.SALUTATIONS_CODE_NEW[salutationPosition]);
      ...
      strt.add(inputAddress.getText().toString());
...
//        .setSingleChoiceItems(Constants.SALUTATIONS_NEW, salutationPosition,
//            (dialog, item) -> {
//              salutationPosition = item;
//              inputSalutation.setText(Constants.SALUTATIONS_NEW[item]);
//              dialog.dismiss();
//            })
//        .show();
//  }
